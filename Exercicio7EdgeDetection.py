...
def on_trackbar_th(val):
    ...
    th = val/100.0
    ret, thresholded_gradient = cv2.threshold(gradient, th, 255, 0)
    cv2.imshow("Moedas Thresholded Gradient", thresholded_gradient)





cv2.namedWindow("Moedas Thresholded Gradient")
cv2.createTrackbar("th", "Moedas Thresholded Gradient", int(th*100), 100,
                   on_trackbar_th)
on_trackbar_th(int(th*100))
#cv2.imshow("Moedas Thresholded Gradient", thresholded_gradient)
